...
const calculateSize = (str, { fontSize = 16, font }) => {
...
class StepCompleter {
  ...
  _resizePopup = (editor, completions) => {
    ...
    const strings = completions.map(comp => {
      comp.caption &&
      comp.caption.length > longestString.length &&
      (longestString = comp.caption)
    ...
    editor.completer.popup.container.style.width = `${width + 200}px`
  }
